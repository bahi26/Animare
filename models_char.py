...
girl_synonymy =['girl','daughter','sister']
boy_girl_synonymy=['child','kid','infant','toddler']

man_synonymy =['man','father','husband','gentleman','player']
woman_synonymy=['woman','lady','mother','wife','gentlewoman']
man_woman_synonymy=['teacher','doctor','transient']
...
##########################################################
determiners_newObject = ['a','an','another','his','her','their','some']
        ...
        for word in sent:
            if word.pos_ == "NOUN":
                ...
                if (model_type == 'boy' or model_type == 'girl' or model_type == 'man' or model_type == 'woman'):
                    object_chars = [-1,"none",-1]    # not mentioned age, not mentioned hair color , not mentioned height , no.of the same object
                else:
                    object_chars = ["none",-1]  # not mentioned color, not mentioned size , no.of the same object
                ...
                for child in word.children:
                    ...
                    ########################### find hair color #####################################
                    ...
                    elif child.dep_ == "prep":   # to describe hair color  --> a boy with brown hair
                        ...
                        if (model_type == 'boy' or model_type == 'girl' or model_type == 'man' or model_type == 'woman'):
                            ...
                            for child_word_with in word_with.children:
                                if child_word_with.dep_ == "pobj":
                                    pobj_word = child_word_with

                            found_hairWord = False
                            found_conj = True

                            while (not found_hairWord) and found_conj:

                                found_conj = False

                                if str(pobj_word) == "hair":
                                    word_hair = pobj_word

                                    for child_word_hair in word_hair.children:
                                        if child_word_hair.dep_ == "amod" and check_color(str(child_word_hair)):
                                            object_chars[1] = str(child_word_hair)
                                            found_hairWord = True
                                            break

                                else:
                                    for child_objWord in pobj_word.children:
                                        if child_objWord.dep_ == "conj":
                                            found_conj = True
                                            pobj_word = child_objWord
                ...
                is_newObject = False
                if word.head.pos_ == "VERB" and lemmatizer.lemmatize(str(word.head),'v') == "be" :
                    for verb_child in word.head.children :
                        if verb_child.dep_ == "expl":
                            is_newObject= True
                else:
                    for each_child in word.children:
                        if each_child.pos_ == "DET":
                            if str(each_child).lower() in determiners_newObject:
                                is_newObject = True
