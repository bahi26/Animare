...
G = None
sizes = []
enviros_names=['room','kitchen', 'street', 'playground']
...
def static_positioning():
    ...
    for entry in range(0, len(content) - 1):
        ch = content[entry].split(" ")
        if(ch[1] not in files_names):ch[1] ='default'
        if(ch[3] not in files_names):ch[3] ='default'
    ...
    for index, ob in enumerate(objects):
        ...
        model, height = load_model(ob[0] + element.size)
        model = red_m(model)
        x, y, z = model.shape
    ...
    if(sorted_ob_size[0].name[0] in enviros_names):
        enviro_name = sorted_ob_size[0].name
        dic[enviro_name].setteled = 1
    else :
        ob = ('room', str(len(objects)) )
        objects.append(ob)
        element = Model.object(ob)
        element.size ='_m'
        model, height = load_model('room_m')
        model = red_m(model)
        x, y, z = model.shape
        element.model = Model.Model(x, y, z, model, height)
        element.tx = element.model.dx + 1
        ...
        dic[ob] = element
        sorted_ob_size = sorted(dic.values(), key=sort_model_size, reverse=True)
        if(sorted_ob_size[0].name in enviros_names):
            enviro_name = sorted_ob_size[0].name
            dic[enviro_name].setteled = 1

  
    if (dic[enviro_name].model.dy < (dic[enviro_name].model.dx/2) and dic[enviro_name].model.dy < (dic[enviro_name].model.dz/2)):
